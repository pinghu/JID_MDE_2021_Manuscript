...
```{r}
library('ggplot2')
tiff('MDE_rarefication_curve.tiff', width = 4800, height = 1800, res=300)
ggplot(
...
```{r}
tiff('MDE_observe_shannon_diversity.tiff', width = 4800, height = 1800, res=300)
...
tiff(filename="MDE.shannon.tiff", width=1800, height=800,res=300)
...
print(p1)
dev.off()
tiff(filename="MDE.Observed.tiff", width=1800, height=800,res=300)
...
```{r}
tiff(filename="MDE.MDS_Site.tiff", width=3800, height=1800,res=300)
...
plot(pca_plot)
dev.off()
tiff(filename="MDE.MDS_AgeGrp_Site.tiff", width=6800, height=1800,res=300)
...
tiff(filename="MDE.MDS_Site_AgeGrp.tiff", width=6800, height=1800,res=300)
...
tiff(filename="MDE.MDS_AgeGrp.tiff", width=3800, height=1800,res=300)
pca2 = phyloseq::plot_ordination(phylo, treat_pca, color = "AgeGroup") + theme_bw(24) + geom_point(size = 3)+ 
  stat_ellipse(geom="polygon", aes(fill = Site), 
                      alpha = 0.05,
                      show.legend = FALSE, 
...
```
```{r}
KP_Shannon_AgeGrp=kruskal.test(Shannon ~ AgeGroup, data = alpha_table)$p.value
KP_Shannon_Site=kruskal.test(Shannon ~ Site, data = alpha_table)$p.value
KP_Shannon_SiteAgeGrp=kruskal.test(Shannon ~ SiteAgeGrp, data = alpha_table)$p.value
Ma<- aggregate(alpha_table$Shannon ~ alpha_table$SiteAgeGrp, FUN=mean)
Mc<-aggregate(alpha_table$Shannon ~ alpha_table$Site, FUN=mean)
Me<-aggregate(alpha_table$Shannon ~ alpha_table$AgeGroup, FUN=mean)
MShannon=mean(alpha_table$Shannon)
CorP_Shannon_Age<-cor.test(alpha_table$Shannon, as.numeric(alpha_table$Age), method="spearman", use="pairwise.complete.obs")$p.value
Cor_Shannon_Age<-cor(alpha_table$Shannon, as.numeric(alpha_table$Age), method="spearman", use="pairwise.complete.obs")
print("KP_Shannon_AgeGrp")
print(KP_Shannon_AgeGrp)
print("KP_Shannon_Site")
print(KP_Shannon_Site)
print("KP_Shannon_SiteAgeGrp")
print(KP_Shannon_SiteAgeGrp)
print("CorP_Shannon_Age")
print(CorP_Shannon_Age)
print("Cor_Shannon_Age")
print(Cor_Shannon_Age)
print(Ma)
print(Mc)
print(Me)
print(MShannon)
for(xxx in unique(alpha_table$Site)){
  data_i=alpha_table[alpha_table$Site == xxx,]
  print(xxx)
  print("KPShannonAgeGroup") 
  print (kruskal.test(Shannon ~ AgeGroup, data=data_i)$p.value)
  print ("CorPShannonAge")
  print(cor.test(data_i$Shannon, as.numeric(data_i$Age), method="spearman", use="pairwise.complete.obs")$p.value)
  print( "CorShannonAge")
  print(cor(data_i$Shannon, as.numeric(data_i$Age), method="spearman", use="pairwise.complete.obs"))     
}
         
```
```{r}
KP_Observed_AgeGrp=kruskal.test(Observed ~ AgeGroup, data = alpha_table)$p.value
KP_Observed_Site=kruskal.test(Observed ~ Site, data = alpha_table)$p.value
KP_Observed_SiteAgeGrp=kruskal.test(Observed ~ SiteAgeGrp, data = alpha_table)$p.value
Ma<- aggregate(alpha_table$Observed ~ alpha_table$SiteAgeGrp, FUN=mean)
Mc<-aggregate(alpha_table$Observed ~ alpha_table$Site, FUN=mean)
Me<-aggregate(alpha_table$Observed ~ alpha_table$AgeGroup, FUN=mean)
MObserved=mean(alpha_table$Observed)
CorP_Observed_Age<-cor.test(alpha_table$Observed, as.numeric(alpha_table$Age), method="spearman", use="pairwise.complete.obs")$p.value
Cor_Observed_Age<-cor(alpha_table$Observed, as.numeric(alpha_table$Age), method="spearman", use="pairwise.complete.obs")
print("KP_Observed_AgeGrp")
print(KP_Observed_AgeGrp)
print("KP_Observed_Site")
print(KP_Observed_Site)
print("KP_Observed_SiteAgeGrp")
print(KP_Observed_SiteAgeGrp)
print("CorP_Observed_Age")
print(CorP_Observed_Age)
print("Cor_Observed_Age")
print(Cor_Observed_Age)
print(Ma)
print(Mc)
print(Me)
...
for(xxx in unique(alpha_table$Site)){
  data_i=alpha_table[alpha_table$Site == xxx,]
  print(xxx)
  print("KPObservedAgeGroup") 
  print (kruskal.test(Observed ~ AgeGroup, data=data_i)$p.value)
  print ("CorPObservedAge")
  print(cor.test(data_i$Observed, as.numeric(data_i$Age), method="spearman", use="pairwise.complete.obs")$p.value)
  print( "CorObservedAge")
  print(cor(data_i$Observed, as.numeric(data_i$Age), method="spearman", use="pairwise.complete.obs"))     
}
