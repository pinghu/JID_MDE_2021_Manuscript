...
#p2A<-ggplot()+ geom_bar(aes(y = Relative Abundance, x = Group, fill = Genus), data = testdata,stat="identity")+scale_fill_manual(values=cols)+theme(legend.position="bottom", legend.direction="horizontal")+coord_flip()
...
tiff(filename="Fig2.tiff", width=4800, height=5800,res=300)
